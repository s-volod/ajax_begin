...
document.querySelector('.ajax-get-html').addEventListener('click', getAjaxHtml);
...
function fetchHtml() {
    fetch('client-data.html').then(response=>response.text()).then(html=>document.querySelector('.html-container').innerHTML = html);
}

// JSON
document.querySelector('.ajax-get-json').addEventListener('click', getAjaxJson);
...
function fetchJson() {
    fetch('client-data.json').then(response=>response.json()).then(clientData=>{
        document.querySelector('.client-name').innerText = clientData.name;
        document.querySelector('.account-balance').innerText = clientData.balance;
    }
    );
